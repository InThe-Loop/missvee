...
class CartController extends Controller {
    ...
    public function store() {
        ...
        if($duplicates->isNotEmpty()) {
            session()->flash('success', 'This item is already in your shopping cart');
    ...
    public function update($id) {
        ...
        session()->flash('error', 'There are NOT enough items left in stock!');
    ...
    public function destroy($id, $cart) {
        ...
        session()->flash('success', 'The item has been removed from the the shopping cart');
        return back();
    }
    ...
    public function saveLater($id) {
        ...
        if($duplicates->isNotEmpty()) {
            session()->flash('success', 'This item is already in your wishlist');
            return redirect()->route('cart.index');
        }
        Cart::instance('saveForLater')->add($item->id, $item->name, 1, $item->price)->associate('App\Product');
        session()->flash('success', 'The item has been saved to your wishlist');
        return redirect()->route('cart.index');
    }
    ...
    public function addToCart($id) {
        ...
        if($exist->isNotEmpty()) {
            session()->flash('success', 'This item is already in the shopping cart. Rather please increase the quantities?');
            return redirect()->route('cart.index');
        }
